...
export default {
  ...
  methods: {
    async checkBalance() {
      ...
      const response = await axios.get(`${process.env.VUE_APP_BACKEND_URL}/transaction/infura/historicalEthBalance/address=${this.address}&timestamp=${this.selectedDate}`);

      const response2 = await axios.get(`${process.env.VUE_APP_BACKEND_URL}/transaction/infura/historicalUsdtBalance/address=${this.address}&timestamp=${this.selectedDate}`);
